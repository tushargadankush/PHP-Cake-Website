<?php include('partials/menu.php')?> 

<div class="main-content">
    <div class="wrapper">
        <h1>Update Admin</h1>
        <br>
        <br>

        <?php
            //1. GET A ID OF SELECTED ADMIN
            $id= $_GET['id'];
            
            //2. CREATE ASQL QUERY TO SEE DETAILS
            $sql= "SELECT * FROM tbl_admin WHERE id=$id";
            
            //3. exwcute thw query
            $res= mysqli_query($conn, $sql);

            //check wheatgher the query is executed or not
            if($res==true){
                //checking whether data is available or not
                $count= mysqli_num_rows($res);

                //check whether we have admin data or not
                if($count==1){
                    //get details
                    //echo "Admin Available";
                    $rows= mysqli_fetch_assoc($res);

                    $full_name=$rows['full_name'];
                    $username=$rows['username'];

                }
                else{
                    //redirect to manage admin page
                    header("location:".SITEURL.'admin/manage-admin.php');
                }
            }
        
        ?>


        <form action="" method="POST">

            <table class="tbl-30">
            <tr>
                    <td>Full Name:</td>
                    <td><input type="text" name="full_name" value= "<?php echo $full_name;?>" > </td>
                </tr>
                <tr>
                    <td>Username:</td>
                    <td><input type="text" name="username" value= "<?php echo $username;?>"></td>
                </tr>

                <tr>
                    <td colspan="2">
                        <input type="hidden" name="id" value="<?php echo $id;?>">
                        <input type="submit" name="submit" value="Update Admin" class="btn-secondary">
                    </td>
                </tr>
            </table>


        </form>
    </div>
</div>

<?php
    // check whether submit button click or not
    if(isset($_POST['submit']))
    {
       // echo " button clicked";
       //get all valus from form 
       $id = $_POST['id'];
       $full_name = $_POST['full_name'];
       $username = $_POST['username'];
     // create asql querry to updatea dmin
     $sql= "UPDATE tbl_admin SET
     full_name='$full_name',
     username='$username'
    WHERE id='$id'
    ";

    //execute the query
        //3. execute querry and save data in datbase
    $res= mysqli_query($conn,$sql) or die(mysqli_error());

    //check whether querry exwcuted or not
    if($res== true)
    {
        // querry excuted successfully and update a admin
        echo "Admin Update";
        //create sessiom variable to dispaly message
        $_SESSION['update']="<div class='success'>Admin Update Successfully </div>";
        //redirect to manage admin page
        header("location:".SITEURL.'admin/manage-admin.php');
    }
    else
    {
        // failrd to  deletw  admin
        echo "Admin Not Update";
        $_SESSION['update']= "<div class='error'>Failed To Update Admin, Try Again Later </div>";
        header("location:".SITEURL.'admin/manage-admin.php');
    }
}


?>

<?php include('partials/footer.php')?> 